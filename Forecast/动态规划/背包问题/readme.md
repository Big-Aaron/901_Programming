...
![img.png](https://img-blog.csdnimg.cn/20210117171307407.png)
...
dp[i] 是通过 dp[i-1] 来得到的，所以我们在得到 dp[i] 后 dp[i-1] 就没有用了。所以可以将二维数组坍缩为一维数组。
...
其他数据会在遍历中被重新赋值，所以赋值为啥都行，所以赋值为0；
...
# **完全背包**
    有 N 件物品和一个最多能被重量为 W 的背包。
    第 i 件物品的重量是 weight[i]，得到的价值是 value[i]。
    每件物品都有无限个（也就是可以放入背包多次），求解装入背包里物品价值总和最大值是多少？

完全背包和01背包问题唯一不同的地方就是，每种物品有无限件。

在 0 - 1 背包的一维滚动数组确定遍历顺序时，我们否定了从前到后遍历，因为从前向后遍历会使得物品被重复放置，而 0-1 背包每件物品只能放一次，
所以只能从后往前遍历。

但在完全背包中，物品可以被重复放置（因为每件物品都有无限个），所以在 0-1 背包的一维滚动数组遍历时从前往后遍历，即为完全背包。

那二重遍历时，应该先遍历物品，还是先遍历背包。

在完全背包中，对于一维dp数组来说，其实两个for循环嵌套顺序跟0-1背包的二维数组遍历顺序同样无所谓！
...
### **6、C语言代码**
[backpack_complete.c](./backpack_complete.c)


# **多重背包**

    有 N 种物品和一个容量为 V 的背包。
    第 i 种物品最多有Mi件可用，每件耗费的空间是 Ci，价值是 Wi。
    求解将哪些物品装入背包可使这些物品的耗费的空间总和不超过背包容量，且价值总和最大。

每件物品最多有 Mi 件可用，把 Mi 件摊开，其实就是一个 0-1 背包问题了。
...
这就变成了 0-1 背包问题了（每个物品只用一次）。

### **6、C语言代码**
[backpack_multiple.c](./backpack_multiple.c)